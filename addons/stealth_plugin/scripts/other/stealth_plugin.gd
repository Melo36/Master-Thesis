...
func _enter_tree() -> void:
	...
	if wizard == null:
		wizard = preload("res://addons/stealth_plugin/scenes/setup_wizard.tscn").instantiate()
...
func _on_apply_button_guard_pressed() -> void:
	...
	_replace_animations(guard, guardAnimationButtonList, assign_3D_button_guard, "res://addons/stealth_plugin/animations/default_guard/")
...
func _get_or_create_single_player(scene: Node) -> Node:
	...
	var player := preload("res://addons/stealth_plugin/scenes/player.tscn").instantiate()
	player.name = "Player"
...
func _create_guard(scene: Node) -> Node:
	...
	var guard := preload("res://addons/stealth_plugin/scenes/guard.tscn").instantiate()
	guard.name = "Guard"
...
func _replace_animations(agent: Node, animationList: Array, model_button : Button, default_anim_path: String):
	...
	var unique_lib_path = agent.lib_path + "_" + agent.get_parent().name + ".res"
	var save_path : String = unique_lib_path
