...
@export var lib_path : String = "res://addons/stealth_plugin/animations/player_animation_library"
...
func _ready():
	...
	if animationTree:
		...
		var unique_lib_path = lib_path + "_" + get_parent().name + ".res"
		if ResourceLoader.exists(unique_lib_path):
			var loaded_library: AnimationLibrary = load(unique_lib_path)
